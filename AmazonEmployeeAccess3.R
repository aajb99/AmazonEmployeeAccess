...
my_recipe <- recipe(rFormula, data = data_train) %>% # set model formula and dataset
  ...
  step_pca(all_predictors(), threshold = 0.85) %>% # Threshold between 0 and 1, test run for classification rf
...
vroom_write(amazon_predictions, "./data/amazon_pred_rf5.csv", delim = ",")
